...
| 260716-wgs | Fatura paga vira RECEITA no /financeiro — módulo receita.ts (transação receita/mensalidade/pago, dedup por marcador [cobranca:id] em notas, sem migration) chamado no webhook Asaas (paga registra; estorno remove) e no PIX manual; backfill da fatura do teste E2E (R$1.500) executado direto no banco; E2E completo do Asaas sandbox VALIDADO nesta sessão (assinatura→cobrança→pagamento→webhook→paga) — atenção: webhook precisa estar ATIVO no painel Asaas (evento gerado com webhook desativado se perde, não há reconciliação) | 2026-07-17 | 88e207e | [260716-wgs-fatura-paga-vira-receita-no-financeiro-t](./quick/260716-wgs-fatura-paga-vira-receita-no-financeiro-t/) |
