...
**Domain:** Internal B2B ops dashboard for a paid-traffic agency (multi-tenant-by-client data, Meta Ads + Google Ads API integrations, scheduled polling, in-app alerts, automated weekly reports, basic financial/MRR tracking). Small internal team (5-10 users), no client-facing portal.
**Researched:** 2026-07-10
**Confidence:** HIGH for core framework/DB/hosting choices, MEDIUM for background-job orchestrator choice, MEDIUM-HIGH for Meta/Google Ads integration approach (API mechanics verified against current docs; exact SDK version pinning changes frequently).
...
npm install facebook-nodejs-business-sdk   # or skip and use fetch + zod directly
...
# Dev dependencies
npm install -D vitest @types/node supabase
```
...
This is the highest-risk, most novel part of the stack, so it's broken out explicitly as requested.
...
- **Auth flow to use:** Because the agency itself manages all ~10 clients' ad accounts inside its own Meta **Business Manager** (this is how agencies operate on Meta — client accounts are added as shared/managed ad accounts, not separately owned apps), you do **not** need a per-client OAuth consent screen. Instead:
  1. Create a **System User** inside the agency's Business Manager (Business Settings → Users → System Users).
  2. Assign that System User access to every client ad account that's shared with the Business Manager.
  3. Generate a **System User access token** — unlike normal user tokens (short-lived: ~1-2h, or long-lived: 60 days requiring rotation), a System User token does not expire on a fixed cycle and is Meta's documented best practice for exactly this use case: unattended, scheduled, server-to-server automation. Store it as an encrypted server-side secret (never exposed to the client).
...
- **Auth flow to use:** Similarly, if the agency's client accounts are linked under the agency's own **Manager account (MCC)**, a **single** OAuth authorization by the agency owner (one-time consent flow) yields one refresh token that — combined with the MCC's `login-customer-id` header — can query every linked client account. No per-client OAuth needed in v1.
  1. Agency owner completes the OAuth consent screen once (Google Cloud OAuth client + `access_type=offline` to receive a refresh token).
  2. Store the refresh token server-side (encrypted); the `google-ads-api` npm client exchanges it for access tokens automatically per request.
  3. Set `login-customer-id` to the MCC ID and target each client account by its `customer_id` in queries.
...
Both pulls (Meta daily status/budget check, Google Ads daily status/budget check, and the Monday weekly-report aggregation across both) should be implemented as **Inngest scheduled functions** (`cron` trigger) calling into dedicated Route Handlers. This gives per-run logs/retries out of the box and avoids hand-rolling a queue for what is, at this client volume, a small and predictable daily workload.

## Stack Patterns by Variant

**If the team later wants intraday alerts (not just once-daily budget checks):**
...
**If the team decides to onboard clients whose ad accounts are NOT already under the agency's Business Manager / MCC:**
...
**If financial/MRR reporting grows beyond simple per-client contract values:**
...
**If the team wants a more polished internal-tool auth experience (SSO, per-user audit trail) later:**
...
---
*Stack research for: Internal agency ops platform (paid-traffic agency, Brazil, ~10 clients, 5-10 internal users)*
*Researched: 2026-07-10*
